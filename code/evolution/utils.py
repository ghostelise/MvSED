...
def SBERT_embed(s_list, language = 'English'):
    ...
    '''
    if language == 'English':
        model = SentenceTransformer('all-MiniLM-L6-v2', device='cuda:1') # for English
    elif language == 'French':
        model = SentenceTransformer('distiluse-base-multilingual-cased-v1') # for French
    embeddings = model.encode(s_list, device='cuda:0', convert_to_tensor = True, normalize_embeddings = True)
