...
def search_stable_points(embeddings, max_num_neighbors = 20):
    ...
    for i in range(max_num_neighbors):
        ...
        knn_edges = [(s+1, d+1, corr_matrix[s, d]) \
            for s, d in enumerate(dst_ids) if corr_matrix[s, d] > 0]
        if i == 0:
            g = nx.Graph()
    ...
    if len(stable_indices) == 0:
        ...
        print('First stable point: k = ', stable_indices[0]+1, ', correspoding 1dSE: ', stable_SEs[0])
        print('Global stable point within the searching range: k = ', index + 1, \
            ', correspoding 1dSE: ', all_1dSEs[index])
    return stable_indices[0]+1, index + 1
...
def hier_2D_SE_mini(block_n, weighted_global_edges, n_messages, n = 100):
    '''
    ...
    while True:
        ...
        for i, subgraph_edges in enumerate(all_subgraphs_edges):
            ...
            seg.update_division_MinSE(block_n)
